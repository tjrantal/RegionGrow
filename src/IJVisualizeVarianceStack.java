...
public class IJVisualizeVarianceStack implements PlugIn {
    ...
    public void run(String arg) {
		...
		for (int d = 0; d < depth; ++d) {
			...
			double[][] lbpImage = Filters.getVarianceImage(tempData, 2);
			for (int r = 0;r<height;++r){
				for (int c = 0;c<width;++c){
		...
		for (int r = 0;r<height;++r){
			...
			double[][] lbpImage = Filters.getVarianceImage(tempData, 1);
			for (int d = 0; d < depth; ++d) {
				for (int c = 0;c<width;++c){
		...
		visualizationStack.setDisplayRange(0,2000);
		...
		horizontalStack.setDisplayRange(0,2000);
