...
public class IJGrowerHorizontal implements PlugIn {
    ...
    public void run(String arg) {
		...
		/*Construct variance image Stack*/
		double[][][] variance3D = new double[width][height][depth];	/*Initialized to zero by Java as default*/
		...
		for (int d = 0; d < depth; ++d) {
			...
			double[][] varianceImage = Filters.getVarianceImage(tempData, 2);
			for (int r = 0;r<height;++r){
				for (int c = 0;c<width;++c){
					variance3D[c][r][d] = varianceImage[c][r];
				}
			}
        }
		
		/*Construct variance image stack on the other direction...*/
		double[][][] varianceHorizonal3D = new double[width][height][depth];	/*Initialized to zero by Java as default*/
		tempData = new double[width][depth];
		IJ.log("Start getting horizontal variance image");
		for (int r = 0;r<height;++r){
			for (int d = 0; d < depth; ++d) {
				for (int c = 0;c<width;++c){
			...
			for (int d = 0; d < depth; ++d) {
				for (int c = 0;c<width;++c){
					varianceHorizonal3D[c][r][d] = varianceImage[c][d];
				}
			}
        }
		IJ.log("Got horizontal variance image");
		...
		if (threeD){	/*3D region grow*/
			...
			double[][] varianceData;
			double[] meanAndArea;
			double[] varianceMeanAndArea;
			double stDev;
			double varStDev;
			meanAndArea = RegionGrow.getCurrentMeanAndArea(mask3D, image3D);
			varianceMeanAndArea = RegionGrow.getCurrentMeanAndArea(mask3D, variance3D);
			if (stdGrow){
				stDev		= RegionGrow.getStdev(mask3D, image3D,meanAndArea[0]);
				varStDev	= RegionGrow.getStdev(mask3D, variance3D,varianceMeanAndArea[0]);
				diffLimit	= 3.0*(stDev+varStDev);
			}
			IJ.log("Mean "+meanAndArea[0]+" DiffLimit "+diffLimit+" varMean "+varianceMeanAndArea[0]);
			...
			for (int d = 0; d < depth; ++d) {
				...
				varianceData= new double[width][height];
				maskHasPixels =false;
				for (int r = 0;r<height;++r){
					for (int c = 0;c<width;++c){
						sliceData[c][r]		= image3D[c][r][d];
						sliceMask[c][r]		= mask3D[c][r][d];
						varianceData[c][r]	= variance3D[c][r][d];
						if (sliceMask[c][r] ==1){
							maskHasPixels = true;
						}
					}
				...
				if (maskHasPixels){ /*Do the remaining steps only if a pixel existed within the slice...*/
					...
					RegionGrow2DVariance rg = new RegionGrow2DVariance(sliceData,sliceMask,varianceData,diffLimit,meanAndArea[0],(long) meanAndArea[1],varianceMeanAndArea[0]);
			...
			if(growUpDown){
				...
				varianceMeanAndArea = RegionGrow.getCurrentMeanAndArea(segmentationMask, varianceHorizonal3D);
				if (stdGrow){
					stDev		= RegionGrow.getStdev(segmentationMask, image3D,meanAndArea[0]);
					varStDev	= RegionGrow.getStdev(segmentationMask, varianceHorizonal3D,varianceMeanAndArea[0]);
					diffLimit	= 3.0*(stDev+varStDev);
				}
				IJ.log("Mean "+meanAndArea[0]+" DiffLimit "+diffLimit+" varMean "+varianceMeanAndArea[0]);
				//IJ.log("Into UD");
				...
				for (int r = 0;r<height;++r){
					...
					varianceData= new double[width][depth];
					/*Get the slice*/
					maskHasPixels = false;
					for (int d = 0; d < depth; ++d) {
						for (int c = 0;c<width;++c){
							...
							varianceData[c][d]	= varianceHorizonal3D[c][r][d];
							if (sliceMask[c][d] ==1){
								maskHasPixels = true;
							}
						}
				...
				if (secondGrow){
					meanAndArea = RegionGrow.getCurrentMeanAndArea(segmentationMask, image3D);
					varianceMeanAndArea = RegionGrow.getCurrentMeanAndArea(segmentationMask, variance3D);
					if (stdGrow){
						stDev		= RegionGrow.getStdev(segmentationMask, image3D,meanAndArea[0]);
						varStDev	= RegionGrow.getStdev(segmentationMask, variance3D,varianceMeanAndArea[0]);
						diffLimit	= 3.0*(stDev+varStDev);
					}
					IJ.log("Mean "+meanAndArea[0]+" DiffLimit "+diffLimit+" varMean "+varianceMeanAndArea[0]);
					threads.clear();
	...
	public class MultiThreader extends Thread{
		public RegionGrow2DVariance r2d;
		public int r;
		private int erodeReps;
		private int preErodeReps;
		private boolean preErode;
		/*Costructor*/
		public MultiThreader(RegionGrow2DVariance r2d, int r,int erodeReps){
			this.r2d = r2d;
			this.r = r;
		...
		public MultiThreader(RegionGrow2DVariance r2d, int r,int erodeReps,int preErodeReps){
			this.r2d = r2d;
			this.r = r;
			this.erodeReps = erodeReps;
