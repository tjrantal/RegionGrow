...
//	sourceDir = "C:\\MyTemp\\oma\\Timon\\tyo\\SubchondralPilot\\karsittu\\kh1\\"
	analyseFiles(sourceDir,dumpDir);
	
	//setBatchMode(false);
}

	//FUNCTION TO ANALYSE FILES
	function analyseFiles(sourceDir,dumpDir){
		...
		//for (i = 9; i<files.length;++i){
			...
			if (File.isDirectory(sourceDir+files[i])==1){ /*A folder*/
				...
				IJ.log(testi2);
				analyseFiles(testi2,dumpDir);
			}else{ /*A File*/
				analyseFile(sourceDir,files[i],dumpDir);
				i = files.length+1;	/*Break the loop. Just the first file in each directory is considered*/
			}
		}
	}
	
	function analyseFile(sourceDir,file,dumpDir){
			run("Image Sequence...", "open="+sourceDir+file+" number=19 starting=1 increment=1 scale=100 file=[] or=[] sort");
			run("Median...", "radius=2 stack");
			run("Seeded 3D Region Grow With LBP", "xlow=169 xhigh=250 ylow=370 yhigh=390 zlow=9 zhigh=12 maxdiff=1.5 secondgrow stdgrow lbplimit1=0.12 lbplimit2=0.12 greylimit1=1.5 greylimit2=3.0 gradientlimit1=1.5 gradientlimit2=1.5 result_dump_path="+dumpDir+file+".mat");
			/*
			run("3D Viewer");
